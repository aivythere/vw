...
class DepositScreen(MDScreen):
    ...
    def __init__(self, screen_manager):
        ...
        self.MinDepText = bfont.MSFont(text=f'Минимальная сумма вклада - {dt.MoneyData(100).AM_TEXT}',
        ...
        self.ReplenishPlaceholder = MDCard(
            bfont.MSFont(text="Пополнить", size="20sp", halign='center', style='Bold'),
            md_bg_color = palette.blued_gray_main_rgba,
            radius = appconf.CARD_RADIUS,
            on_release=lambda *a: print('topup sc'),  # elements.change_screen(self.scr, "TopUp")
        ...
        main_grid.add_widget(self.ReplenishPlaceholder)
        main_grid.add_widget(self.MinDepText)
    ...
    # def clear(self, *args):
    #     self.sum_textfield.text_field.text = self.slider.value
    ...
    def on_pre_enter(self, *args):
        ...
        self.REQUEST_INSTANCE = Clock.schedule_once(lambda *a: UrlRequest(url=appconf.SERVER_DOMAIN,
                                                                          req_body=json.dumps({'method': 'OV_lookup',
                                                                                               'my_id': self.MYID,
                                                                                               'dep_period': self.DEP_PERIOD}),
                                                                          on_success=self.success_lookup,
                                                                          on_error=self.error_lookup,
    ...
    def ReallyOpenDep(self, *args):

        if self.POPUP:
            self.POPUP.disabled = True
            self.POPUP.dismissable = False

        def odSuccess(*args):
            r = json.loads(args[-1])
            self.POPUP.dismiss(force_close=True)  # force_close - мой аргумент, не встроенный

            if r['response'] == "TRUE":
                print('deposit successfull')
                self.on_pre_enter()  # костыльная перезагрузка элементов

                import successfull_deposit_screen as sd
        ...
        UrlRequest(url=appconf.SERVER_DOMAIN,
                   req_body=json.dumps({'method': 'open_deposit', 'my_id': self.MYID, 'dep_period': self.DEP_PERIOD,
                                        'dep_sum': self.DEP_SUM}),
                   on_success=odSuccess, on_error=odError,
                   timeout=appconf.REQUEST_TIMEOUT,
    ...
    def openDeposit(self, *args):
        self.DEP_SUM = self.slider.value

        if self.BALANCE >= self.DEP_SUM >= 100:
            self.POPUP = elements.ApproveBuyPopup(sum=dt.MoneyData(self.DEP_SUM).AM_TEXT,
                                                  per=self.DEP_PERIOD,
                                                  prf=dt.MoneyData(
                                                      elements.cash(self.DEP_SUM, self.PERCENT, True)).AM_TEXT,
                                                  pyo=dt.MoneyData(elements.cash(self.DEP_SUM, self.PERCENT)).AM_TEXT,
                                                  date=elements.convert_time(self.DEP_PERIOD),
                                                  baf=self.ReallyOpenDep,
                                                  radius=[appconf.CARD_RADIUS for _ in range(4)])
            self.POPUP.open()
        elif self.DEP_SUM < 100:
            animations.uplight_text().start(self.MinDepText)
        elif self.DEP_SUM > self.BALANCE:
            animations.card_change_bg(palette.card_unavailable_red).start(self.BALANCE_card)
    ...
    def success_lookup(self, *args):
        ...
        if response['period'] == self.DEP_PERIOD:
            ...
            if self.BALANCE <= 100:
                animations.upscale_placeholder().start(self.ReplenishPlaceholder)
    ...
    def error_lookup(self, *args):
        ...
        if self.REQUEST_ERR_COUNT >= appconf.REQUEST_ERR_COUNTOUT:
            ...
            Clock.schedule_once(lambda *a: UrlRequest(url=appconf.SERVER_DOMAIN,
                                                      req_body=json.dumps({'method': 'OV_lookup', 'my_id': self.MYID,
                                                                           'dep_period': self.DEP_PERIOD}),
